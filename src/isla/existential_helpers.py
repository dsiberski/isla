...
def insert_tree(
    ...
    # TODO: do this elsewhere, no children[0}
    possible_rules = identify_rule_expansions(grammar, in_tree.value, possible_parents[0])

    result = []

    for idx in insertion_points:
        result.append(create_new_tree(tree, in_tree, idx, possible_rules))
...
def identify_insertion_points(tree: DerivationTree, parent_types: List[str]):
    # TODO: check for different data structure instead of list, maybe deque for nodes
    """Checks all nodes for possible insertion points in DFS order and return the corresponding node ids as a list."""
    # TODO: should not return positions, might return path as well
    ...
    while nodes:
        ...
        if current_node is not None:
            ...
            if current_node.value in parent_types:
                interesting_nodes.append(current_node.id)  # TODO: get rid of second part
...
def insert_tree_into_parent(old_tree: DerivationTree, tree_to_insert: DerivationTree, possible_rule: List[List[str]]) \
        -> DerivationTree:
    """Returns tree where tree_to_insert has been inserted as nth child into the tree."""

    children = old_tree.children
    new_children = children[:child_n] + (tree_to_insert,) + children[child_n:]  # TODO: do the grammar stuff

    if tree_to_insert.is_open is True or tree_to_insert.children is None:  # TODO: does this cover all options?
        is_open = True
    ...
    return DerivationTree(old_tree.value, new_children, id=old_tree.id, is_open=is_open)
